...
class GpsUtil {
  static Map<String, double?> _locationMap = {'longitude': -1, 'latitude': -1, 'accuracy': -1, 'altitude': -1, 'speed': -1, 'speed_accuracy': -1};
  ...
  static Future<void> onStream({interval = 10000}) async {
    ...
    if (hasPermission) {
      FlLocation.getLocationStream(interval: interval, accuracy: LocationAccuracy.high).handleError((error) {}).listen((event) {
