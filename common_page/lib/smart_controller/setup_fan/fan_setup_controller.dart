...
class FanSetupController extends GetxController {
  ...
  void onInit() {
    ...
    boNoSettingFan = ButtonOutline(controller: GetXCreator.putButtonOutlineController("boNoSettingFan"), label: "Tidak", onClick: () => Get.back());

    bfYesSettingFan = ButtonFill(controller: GetXCreator.putButtonFillController("bfYesSettingFan"), label: "Ya", onClick: () => settingFan());
  }
        ...
        if (auth != null) {
          ...
          Service.push(
              ...
              body: [
                ...
                auth.id,
                GlobalVar.xAppId ?? '-',
                ListApi.pathDetailFanData(basePath, "fan", isForPitikConnect ? device.deviceSummary!.coopCodeId! : device.coopId ?? '-', isForPitikConnect ? device.deviceSummary!.deviceId! : device.deviceId ?? '-', deviceSetting.id!)
              ],
        ...
        if (auth != null) {
          ...
          Service.push(
              ...
              listener: ResponseListener(
                  onResponseDone: (code, message, body, id, packet) {
                    isLoading.value = false;
                    Get.off(TransactionSuccessActivity(keyPage: "smartFanSaved", message: "Kamu telah berhasil melakukan pengaturan kipas", showButtonHome: false, onTapClose: () => Get.back(), onTapHome: () {}));
                  },
                  onResponseFail: (code, message, body, id, packet) {
                    isLoading.value = false;
        ...
        builder: (context) {
          return Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Container(
                    margin: const EdgeInsets.only(top: 24, left: 16, right: 16), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Expanded(child: bfYesSettingFan), const SizedBox(width: 16), Expanded(child: boNoSettingFan)])),
