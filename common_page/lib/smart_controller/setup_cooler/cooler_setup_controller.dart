...
class CoolerSetupController extends GetxController {
  ...
  void onInit() {
    ...
    boNoSetCooler = ButtonOutline(controller: GetXCreator.putButtonOutlineController("boNoSetCooler"), label: "Tidak", onClick: () => Get.back());

    bfYesSetCooler = ButtonFill(controller: GetXCreator.putButtonFillController("bfYesSetCooler"), label: "Ya", onClick: () => settingCooler());
        ...
        if (auth != null) {
          ...
          Service.push(
              ...
              body: [
                ...
                auth.id,
                GlobalVar.xAppId ?? '-',
                ListApi.pathDeviceData(basePath, "cooler/detail", isForPitikConnect ? device.deviceSummary!.coopCodeId! : device.coopId ?? '-', isForPitikConnect ? device.deviceSummary!.deviceId! : device.deviceId ?? '-')
              ],
        ...
        if (auth != null) {
          ...
          Service.push(
            ...
            listener: ResponseListener(
                onResponseDone: (code, message, body, id, packet) {
                  isLoading.value = false;
                  Get.off(TransactionSuccessActivity(keyPage: "smartCoolerSaved", message: "Kamu telah berhasil melakukan pengaturan pendingin", showButtonHome: false, onTapClose: () => Get.back(), onTapHome: () {}));
                },
                onResponseFail: (code, message, body, id, packet) {
                  isLoading.value = false;
  ...
  DeviceSetting generatePayloadFanSetup() => DeviceSetting(
      deviceId: controllerData.deviceId, coolingPadTemperature: efTargetTempColler.getInputNumber(), timeOnCoolingPad: tmPickerDurationCoolerOn.getLastTimeSelectedText(), timeOffCoolingPad: tmPickerCoolerOffDuration.getLastTimeSelectedText());
