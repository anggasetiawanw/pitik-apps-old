    ...
    Timer(const Duration(seconds: 6), () async {
      ...
      if (await isValidUrl(uri)) {
          ...
          errorBuilder: (BuildContext context, Object exception, StackTrace? stackTrace) {
            return const SizedBox(
              ...
              child: Column(
                ...
                children: [
                  Center(child: CircularProgressIndicator(color: GlobalVar.primaryOrange)),
                ],
              ),
            );
          },
        ...
        if (tryCount <= 10) {
          ...
          image.value = Image.network(url, fit: BoxFit.fill, loadingBuilder: (BuildContext context, Widget child, ImageChunkEvent? loadingProgress) {
            if (loadingProgress == null) return child;
            return Center(
                child: CircularProgressIndicator(
              color: GlobalVar.primaryOrange,
            ...
            return SizedBox(
                ...
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [SvgPicture.asset("images/wifi_off_icon.svg"), Text("Koneksi terputus gagal mengambil gambar", style: TextStyle(color: GlobalVar.black, fontSize: 14, fontWeight: GlobalVar.medium))]));
          });
        }
      }
    });
