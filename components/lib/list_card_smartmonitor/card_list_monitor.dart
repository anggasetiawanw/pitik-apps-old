  ...
  Widget build(BuildContext context) {
    Widget itemSensor(String title, SensorData? sensorData) {
      return Container(
          ...
          decoration: BoxDecoration(border: Border.all(color: GlobalVar.outlineColor, width: 1), borderRadius: BorderRadius.circular(8)),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              Container(
                  ...
                  child: Text("${sensorData!.value == null ? 0 : sensorData.value}",
                      style: sensorData.status == GOOD
                          ? GlobalVar.greenTextStyle
                          : sensorData.status == BAD
                              ? GlobalVar.redTextStyle
                              : GlobalVar.blackTextStyle.copyWith(fontSize: 14, fontWeight: GlobalVar.bold),
                      overflow: TextOverflow.clip)),
              Container(margin: const EdgeInsets.only(left: 8), child: Text(title == "Temperature" ? "°C" : sensorData.uom ?? "%", style: GlobalVar.primaryTextStyle.copyWith(fontSize: 16), overflow: TextOverflow.clip))
            ])
          ]));
    }
