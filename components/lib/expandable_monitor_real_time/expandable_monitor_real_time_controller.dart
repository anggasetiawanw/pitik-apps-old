...
class ExpandableMonitorRealTimeController extends GetxController {
  ...
  void getRealTimeHistoricalDataForSmartController({required String sensorType, required int day, required String deviceIdForController, required String coopIdForController}) => AuthImpl().get().then((auth) {
        if (auth != null) {
          ...
          _pushToServerForHistoricalData(sensorType: sensorType, route: ListApi.getRealTimeHistoricalForSmartController, bodyRequest: bodyRequest, roomId: '');
        } else {
  ...
  void getRealTimeHistoricalData({required String sensorType, required int day, required Coop coop, required String roomId}) => AuthImpl().get().then((auth) {
        if (auth != null) {
          ...
          _pushToServerForHistoricalData(sensorType: sensorType, route: ListApi.getRealTimeHistorical, bodyRequest: bodyRequest, roomId: roomId);
        } else {
          GlobalVar.invalidResponse();
        }
      });
  ...
  void loadData(Rx<List<GraphLine>> historicalList, String sensorType) {
    if (sensorType == "temperature" || sensorType == "relativeHumidity" || sensorType == "heatStressIndex") {
      ...
      gvSmartMonitoring.controller.setUom(sensorType == "wind" ? 'm/s' : 'Lux').setLineCurrentColor(GlobalVar.primaryOrange).setTextColorCurrentTooltip(GlobalVar.primaryOrange);
    } else if (sensorType == "ammonia") {
      gvSmartMonitoring.controller.setUom('ppm').setLineCurrentColor(GlobalVar.primaryOrange).setTextColorCurrentTooltip(GlobalVar.primaryOrange);
    }
            ...
            child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Padding(
                      ...
                      child: Align(alignment: Alignment.centerLeft, child: Text("Tata letak sensor dalam kandang", style: GlobalVar.subTextStyle.copyWith(fontSize: 21, fontWeight: GlobalVar.bold, color: GlobalVar.primaryOrange)))),
                    ...
                    children: List.generate(sensorPositionList.length, (index) {
                      return Row(
                        children: [
                          ...
                          Text(sensorPositionList[index] == null || sensorPositionList[index]!.position == null ? '-' : sensorPositionList[index]!.position!,
