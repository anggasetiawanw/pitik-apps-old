...
class StockOpnameTwoFieldController extends GetxController {
  ...
  late Expandable exp = Expandable(
    ...
    child: Column(
      children: efSku.value.map((element) {
  ...
  void generateEf(Products product) {
    ...
    for (var element in product.productItems!) {
      efSku.value.add(EditFieldTwoRow(
        controller: GetXCreator.putEditFieldTwoRowController(element!.name!),
        label: element.name!,
        hint: "0",
        alertText: "Kolom ini harus di isi",
        textUnit1: product.quantityUOM!,
        textUnit2: product.weightUOM!,
        maxInput: 50,
        onTyping1: (value, control) {},
        onTyping2: (value, control) {},
