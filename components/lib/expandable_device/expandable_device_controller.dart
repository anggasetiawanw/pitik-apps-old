...
class ExpandableDeviceController extends GetxController {
  ...
  void getHistoricalData(String sensorType, Device device, int day) {
    ...
    Service.push(
        ...
        context: context,
        body: [GlobalVar.auth!.token, GlobalVar.auth!.id, GlobalVar.xAppId!, sensorType, day == 0 ? "" : day, ListApi.pathHistoricalData(device.deviceId!)],
  ...
  void loadData(Rx<List<GraphLine>> historicalList, String sensorType) {
    if (sensorType == "temperature" || sensorType == "relativeHumidity" || sensorType == "heatStressIndex") {
      ...
      gvSmartMonitoring.controller.setLineCurrentColor(GlobalVar.primaryOrange).setTextColorCurrentTooltip(GlobalVar.primaryOrange);
