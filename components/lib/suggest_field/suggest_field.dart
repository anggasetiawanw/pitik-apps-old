...
class SuggestField extends StatelessWidget {
  ...
  SuggestField({required this.controller, this.id = 1, required this.label, required this.hint, this.caseSensitive = false, required this.alertText, this.childPrefix, required this.suggestList, required this.onTyping, required this.onSubmitted});
                ...
                Align(
                    ...
                    child: controller.showTooltip.isTrue
                        ? Container(
                            ...
                            child: Row(children: [Padding(padding: const EdgeInsets.only(right: 8), child: SvgPicture.asset("images/error_icon.svg")), Text(alertText, style: const TextStyle(color: GlobalVar.red, fontSize: 12))]))
                        : Container())
              ]))
        ])));
  }
