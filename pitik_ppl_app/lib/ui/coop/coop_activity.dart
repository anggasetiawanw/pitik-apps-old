        ...
        body: SafeArea(
          ...
          Obx(() => controller.isLoading.isTrue
              ...
              : Expanded(
                  ...
                  RefreshIndicator(
                      onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.generateCoopList(true)),
                      child: RawScrollbar(
                          thumbVisibility: true,
                          thumbColor: GlobalVar.primaryOrange,
                          radius: const Radius.circular(8),
                          child: ListView.builder(physics: const AlwaysScrollableScrollPhysics(), itemCount: controller.coopFilteredList.length, itemBuilder: (context, index) => controller.createCoopActiveCard(index)))),
                  RefreshIndicator(
                      onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.generateCoopList(false)),
                      child: RawScrollbar(
                          thumbVisibility: true,
                          thumbColor: GlobalVar.primaryOrange,
                          radius: const Radius.circular(8),
                          child: ListView.builder(physics: const AlwaysScrollableScrollPhysics(), itemCount: controller.coopFilteredList.length, itemBuilder: (context, index) => controller.createCoopIdleCard(index))))
                ])))
        ])));
  }
}
