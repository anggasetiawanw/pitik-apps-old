...
class DashboardCommon {
  ...
  static void _ascendingHistory(ListSmartScaleController controller, Coop coop, String? startDateCustom) {
    final List<SmartScale?> scalesAscending = [];
    ...
    for (int i = coop.day ?? 0; i >= 1; i--) {
      ...
      final DateTime dateTime = Convert.getDatetime(startDateCustom ?? coop.startDate!).add(Duration(days: i));
      if (!isInsert && !isLast) {
        final SmartScale historyScaleTemp = SmartScale(
              ...
              child: Stack(children: [
                Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(8)), color: customBackground ?? GlobalVar.primaryLight2),
