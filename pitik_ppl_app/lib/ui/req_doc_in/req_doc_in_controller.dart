...
class RequestDocInController extends GetxController {
  ...
  void enableField() {
    ...
    if ((coop.statusText ?? '').toLowerCase() == SUBMISSION_STATUS.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == APPROVED_OVK.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == SUBMITTED_OVK.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == OVK_REJECTED.toLowerCase()) {
      dtTanggal.controller.enable();
      efPopulasi.controller.enable();
      btNext.controller.changeLabel('Simpan');
    } else if ((coop.statusText ?? '').toLowerCase() == SUBMITTED_DOC_IN.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == SUBMITTED_STATUS.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == NEED_APPROVED.toLowerCase() ||
        (coop.statusText ?? '').toLowerCase() == PROSESSING.toLowerCase()) {
      dtTanggal.controller.enable();
      efPopulasi.controller.enable();
      btNext.controller.changeLabel('Simpan');
    }
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [Text('Populasi', style: GlobalVar.greyTextStyle.copyWith(fontSize: 12)), Text('${efPopulasi.getInput()} Ekor', style: GlobalVar.blackTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.bold))],
                      ),
                    ],
                  ),
                ),
  ...
  void processRequest() {
    isLoading.value = true;
    final RequestChickin requestChickinBody = RequestChickin();
    ...
    if ((coop.statusText ?? '').toLowerCase() == SUBMISSION_STATUS.toLowerCase() || (coop.statusText ?? '').toLowerCase() == REJECTED.toLowerCase()) {
      ...
      if (allowApprove.isTrue) {
        final RequestChickin requestChickinApprove = RequestChickin();
