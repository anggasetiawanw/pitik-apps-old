...
class AdjustmentFeedController extends GetxController {
    ...
    void onInit() {
        ...
        // SETUP FEED WIDGET
        feedCategory = SpinnerField(controller: GetXCreator.putSpinnerFieldController("adjustmentFeedCategory"), label: "Kategori Pakan", hint: "Pilih Kategori Pakan", backgroundField: GlobalVar.primaryLight, alertText: "Kategori Pakan harus dipilih..!",
        ...
        feedQuantityField = EditField(controller: GetXCreator.putEditFieldController("adjustmentFeedQuantity"), label: "Total", hint: "Ketik di sini", alertText: "Total belum diisi..!", textUnit: "", maxInput: 20, inputType: TextInputType.number,
            onTyping: (text, field) {}
        );
        ...
        feedMultipleFormField = MultipleFormField<Product>(
            ...
            child: Column(
                children: [
                    feedCategory,
                    feedQuantityField,
                    isLoadingFeedBrand.isTrue ? const Center(child: ProgressLoading()) : feedBrandSpinnerField
                ]
            )
        );
    ...
    Product getFeedSelectedObject() {
        if (feedBrandSpinnerField.controller.getSelectedObject() != null) {
            Product product = feedBrandSpinnerField.controller.getSelectedObject();
                    ...
                    onResponseDone: (code, message, body, id, packet) {
                        ...
                        if ((body as ProductsResponse).data.isNotEmpty) {
                            ...
                            for (var product in body.data) {
                                String key = '${product == null || product.subcategoryCode == null ? '' : product.subcategoryCode} - ${product == null || product.productName == null ? '' : product.productName}';
                                ...
                                children: [
                                    ...
                                    SizedBox(
                                        ...
                                        child: Row(
                                            ...
                                            children: [
                                                Expanded(
                                                    child: ButtonFill(controller: GetXCreator.putButtonFillController("btnAgreeAdjustmentFeed"), label: "Yakin", onClick: () {
                                                        ...
                                                        for (var product in feedMultipleFormField.controller.listObjectAdded.values) {
                                                            body.add({
                                                                'feedStockSummaryId': (product as Product).id,
                                                                'adjustmentQuantity': product.quantity,
                                                                'type': 'Pengurangan',
                                                                'remarks': eaNotes.getInput()
                                                            });
                                                        }
                                                ...
                                                const SizedBox(width: 16),
                                                Expanded(
                                                    child: ButtonOutline(controller: GetXCreator.putButtonOutlineController("btnNotAdjustmentFeed"), label: "Tidak Yakin", onClick: () => Navigator.pop(Get.context!))
                                                )
                                            ]
                                        )
                                    ),
