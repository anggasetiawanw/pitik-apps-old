            ...
            Scaffold(
                ...
                body: Column(
                    children: [
                        ...
                        controller.isLoading.isTrue ? Padding(padding: EdgeInsets.only(top: (MediaQuery.of(context).size.height - 80) / 3), child: const Center(child: ProgressLoading())) :
                        Expanded(
                            child: RawScrollbar(
                                ...
                                child: RefreshIndicator(
                                    onRefresh: () => Future.delayed(
                                        const Duration(milliseconds: 200), () => controller.getFloorList()
                                    ),
                                    child: ListView.builder(
                                        physics: const AlwaysScrollableScrollPhysics(),
                                        itemCount: controller.floorList.length,
                                        itemBuilder: (context, index) => controller.createFloorCard(floor: controller.floorList[index])
                                    ),
                                )
                            )
                        )
