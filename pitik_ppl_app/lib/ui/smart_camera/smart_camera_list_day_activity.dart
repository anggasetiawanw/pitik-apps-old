    ...
    Widget build(BuildContext context) {
        return Obx(() =>
            Scaffold(
                ...
                body: controller.isLoading.isTrue ? // IF LOADING IS RUNNING
                const Center(child: ProgressLoading()) :
                RefreshIndicator(
                    child: controller.dayList.isEmpty ? // IF LOADING IS DONE AND DATA EMPTY
                    Column(
                        children: [
                    ...
                    onRefresh: () => Future.delayed(
                        const Duration(milliseconds: 200), () => controller.getDayList()
                    )
                ),
            )
        );
