...
class HarvestSubmittedDetailController extends GetxController {
  ...
  void onInit() {
    ...
    rejectReasonAreaField =
        EditAreaField(controller: GetXCreator.putEditAreaFieldController('harvestSubmittedRejectReason'), label: 'Alasan', hint: 'Tulis Alasan disini...', alertText: 'Alasan belum diisi..!', maxInput: 250, onTyping: (text, field) {});
  ...
  Widget getStatusWidget() {
    final String statusText = harvest.value == null ? '-' : harvest.value!.statusText ?? '-';
    final Color background = statusText == GlobalVar.PENGAJUAN || statusText == GlobalVar.SUBMITTED
    ...
    final Color textColor = statusText == GlobalVar.PENGAJUAN || statusText == GlobalVar.SUBMITTED
      ...
      containerButtonBottom.value = SizedBox(
        child: Container(
            ...
            child: SizedBox(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Expanded(
                    child: ButtonFill(controller: GetXCreator.putButtonFillController('btnHarvestSubmittedEdit'), label: 'Edit', onClick: () => Get.toNamed(RoutePage.harvestSubmittedForm, arguments: [coop, harvest.value, true])),
                  )
                ]))),
      );
        ...
        child: Container(
            ...
            child: SizedBox(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Expanded(
                      child: ButtonOutline(
                          controller: GetXCreator.putButtonOutlineController('btnHarvestSubmittedReject'),
                          label: 'Tolak',
                          onClick: () {
                            isCancel.value = true;
                            _generateButtonBottom();
      ...
      if (auth != null) {
        Service.push(
            ...
            listener: ResponseListener(
                onResponseDone: (code, message, body, id, packet) {
                  isLoading.value = false;
                  Get.off(TransactionSuccessActivity(keyPage: 'harvestSubmittedApprove${harvest.value!.id}', message: 'Kamu telah menyetujui permintaan panen', showButtonHome: false, onTapClose: () => Get.back(result: true), onTapHome: () {}));
                },
                onResponseFail: (code, message, body, id, packet) {
                  Get.snackbar('Pesan', '${(body as ErrorResponse).error!.message}', snackPosition: SnackPosition.TOP, colorText: Colors.white, backgroundColor: Colors.red);
                  isLoading.value = false;
