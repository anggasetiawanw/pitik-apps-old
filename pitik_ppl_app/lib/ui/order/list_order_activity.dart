            ...
            children: [
              Padding(
                ...
                child: TabBar(
                    ...
                    tabs: const [Tab(text: 'Pengajuan'), Tab(text: 'Diproses'), Tab(text: 'Penerimaan')]),
              ),
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: const BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(6)), color: GlobalVar.primaryLight),
                  ...
                  : Expanded(
                      ...
                      RawScrollbar(
                          ...
                          child: RefreshIndicator(
                            onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.getListRequested()),
                            child: ListView.builder(
                                physics: const AlwaysScrollableScrollPhysics(), itemCount: controller.orderList.length, itemBuilder: (context, index) => controller.createOrderCard(typePosition: 0, procurement: controller.orderList[index])),
                          )),
                      RawScrollbar(
                          thumbColor: GlobalVar.primaryOrange,
                          radius: const Radius.circular(8),
                          child: RefreshIndicator(
                            onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.getListProcessed()),
                            child: ListView.builder(
                                physics: const AlwaysScrollableScrollPhysics(), itemCount: controller.orderList.length, itemBuilder: (context, index) => controller.createOrderCard(typePosition: 1, procurement: controller.orderList[index])),
                          )),
                      RawScrollbar(
                          thumbColor: GlobalVar.primaryOrange,
                          radius: const Radius.circular(8),
                          child: RefreshIndicator(
                            onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.getListReceived()),
                            child: ListView.builder(
                                physics: const AlwaysScrollableScrollPhysics(), itemCount: controller.orderList.length, itemBuilder: (context, index) => controller.createOrderCard(typePosition: 2, procurement: controller.orderList[index])),
                          ))
                    ]))
            ],
          ),
