...
class OrderDetailController extends GetxController {
  ...
  void onInit() {
    ...
    rejectReasonAreaField = EditAreaField(controller: GetXCreator.putEditAreaFieldController('orderRejectReason'), label: 'Alasan', hint: 'Tulis Alasan disini...', alertText: 'Alasan belum diisi..!', maxInput: 250, onTyping: (text, field) {});
    _getDetailRequest();
  }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: procurement.value.statusText == null
              ? Colors.transparent
              : procurement.value.statusText == GlobalVar.SUBMITTED || procurement.value.statusText == GlobalVar.NEED_APPROVAL
      ...
      if (auth != null) {
        Service.push(
            ...
            listener: ResponseListener(
                onResponseDone: (code, message, body, id, packet) {
                  isLoading.value = false;
                  Get.off(TransactionSuccessActivity(keyPage: 'orderApprove${procurement.value.id}', message: 'Kamu telah menyetujui permintaan sapronak', showButtonHome: false, onTapClose: () => Get.back(), onTapHome: () {}));
                },
                onResponseFail: (code, message, body, id, packet) {
                  Get.snackbar('Pesan', '${(body as ErrorResponse).error!.message}', snackPosition: SnackPosition.TOP, colorText: Colors.white, backgroundColor: Colors.red);
                  isLoading.value = false;
  ...
  void _rejectOrder({bool isCancel = false}) {
    if (rejectReasonAreaField.getInput().isNotEmpty) {
      ...
      AuthImpl().get().then((auth) {
        if (auth != null) {
          final String endpoint = isCancel ? 'v2/purchase-requests/${procurement.value.id}/cancel' : 'v2/purchase-requests/${procurement.value.id}/reject';
          Service.push(
              apiKey: 'productReportApi',
            ...
            containerButtonEditAndCancel.value = SizedBox(
              child: Container(
                  ...
                  child: SizedBox(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Expanded(
                          child: ButtonFill(controller: GetXCreator.putButtonFillController('btnOrderEdit'), label: 'Edit', onClick: () => Get.toNamed(RoutePage.orderRequestPage, arguments: [coop, true, fromCoopRest, procurement.value])),
                        ),
                        const SizedBox(width: 16),
                        Expanded(
                            child: ButtonOutline(
                                controller: GetXCreator.putButtonOutlineController('btnOrderCancel'),
                                label: 'Batal',
                                onClick: () {
                                  isCancel.value = true;
              ...
              child: Container(
                  ...
                  child: SizedBox(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        ...
                        Expanded(
                            child: ButtonOutline(
                                controller: GetXCreator.putButtonOutlineController('btnOrderReject'),
                                label: 'Tolak',
                                onClick: () {
                                  isCancel.value = true;
                                  _generateButtonBottom();
