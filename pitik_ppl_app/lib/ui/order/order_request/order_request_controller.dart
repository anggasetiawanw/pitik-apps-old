...
class OrderRequestController extends GetxController {
  ...
  void onInit() {
    ...
    orderDateField = DateTimeField(
        controller: GetXCreator.putDateTimeFieldController('orderDateField'),
        label: 'Tanggal Order',
        hint: '20/02/2022',
        alertText: 'Tanggal Order harus diisi..!',
    ...
    orderTypeField = SpinnerField(
        controller: GetXCreator.putSpinnerFieldController('orderTypeField'),
        label: 'Jenis Order',
        hint: 'Pakan',
        alertText: '',
    ...
    orderMultipleLogisticField = SpinnerField(
        controller: GetXCreator.putSpinnerFieldController('orderMultipleLogisticField'),
        label: 'Digabung?',
    ...
    feedQuantityField =
        EditField(controller: GetXCreator.putEditFieldController('orderFeedQuantity'), label: 'Total', hint: 'Ketik di sini', alertText: 'Total belum diisi..!', textUnit: '', maxInput: 20, inputType: TextInputType.number, onTyping: (text, field) {});
    ...
    ovkQuantityField =
        EditField(controller: GetXCreator.putEditFieldController('orderOvkQuantity'), label: 'Total', hint: 'Ketik di sini', alertText: 'Total belum diisi..!', textUnit: '', maxInput: 20, inputType: TextInputType.number, onTyping: (text, field) {});
    ...
    ovkUnitQuantityField = EditField(
        controller: GetXCreator.putEditFieldController('orderOvkUnitQuantity'), label: 'Total', hint: 'Ketik di sini', alertText: 'Total belum diisi..!', textUnit: '', maxInput: 20, inputType: TextInputType.number, onTyping: (text, field) {});
    ...
    ovkVendorMultipleFormField = MultipleFormField<Product>(
        ...
        padding: const EdgeInsets.only(left: 16, right: 16, bottom: 16),
        decoration: const BoxDecoration(borderRadius: BorderRadius.only(bottomRight: Radius.circular(10), bottomLeft: Radius.circular(10)), border: Border.fromBorderSide(BorderSide(color: GlobalVar.grayBackground, width: 3))),
        labelButtonAdd: 'Tambah OVK',
        initInstance: Product(),
        childAdded: () => _createChildAdded(getOvkProductName(), getOvkQuantity(product: null)),
        increaseWhenDuplicate: (product) => _createChildAdded(getOvkProductName(), getOvkQuantity(product: product)),
    ...
    ovkUnitMultipleFormField = MultipleFormField<Product>(
        ...
        child: Column(children: [ovkUnitSuggestField, ovkUnitQuantityField]));
  ...
  void _initializeFillForm() {
    // fill delivery date
    final DateTime dateTime = Convert.getDatetime(procurement.deliveryDate!);
    ...
    if (isFeed.value) {
      ...
      if (isOwnFarm) {
        ...
        for (var product in procurement.details) {
          ovkVendorMultipleFormField.getController().addData(child: _createChildAdded(getOvkProductName(product: product), getOvkQuantity(product: product)), object: product, key: getOvkProductName(product: product));
        }
        ...
        if (procurement.internalOvkTransferRequest != null) {
          for (var product in procurement.internalOvkTransferRequest!.details) {
            ovkUnitMultipleFormField.getController().addData(child: _createChildAdded(getOvkUnitProductName(product: product), getOvkUnitQuantity(product: product)), object: product, key: getOvkUnitProductName(product: product));
          }
        }
      } else {
        // fill list product
        for (var product in procurement.details) {
          ovkMultipleFormField.getController().addData(child: _createChildAdded(getOvkProductName(product: product), getOvkQuantity(product: product)), object: product, key: getOvkProductName(product: product));
        }
      }
    }
  }
                          ...
                          child: Column(
                            ...
                            return Padding(
                                ...
                                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Expanded(child: Text(getFeedProductName(product: feedMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))),
                                  Text(getFeedQuantity(product: feedMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))
                                ]));
                          }).toList()),
                        )
                      ] else ...[
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                return Padding(
                                    padding: const EdgeInsets.only(top: 8),
                                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Expanded(child: Text(getOvkProductName(product: ovkVendorMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))),
                                      Text(getOvkQuantity(product: ovkVendorMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))
                                    ]));
                              }).toList()),
                            )
                          ],
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                return Padding(
                                    padding: const EdgeInsets.only(top: 8),
                                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Expanded(child: Text(getOvkProductName(product: ovkUnitMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))),
                                      Text(getOvkQuantity(product: ovkUnitMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))
                                    ]));
                              }).toList()),
                            )
                          ]
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              return Padding(
                                  padding: const EdgeInsets.only(top: 8),
                                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Expanded(child: Text(getOvkProductName(product: ovkMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))),
                                    Text(getOvkQuantity(product: ovkMultipleFormField.getController().listObjectAdded[entry.key]), style: TextStyle(color: GlobalVar.black, fontSize: 12, fontWeight: GlobalVar.medium))
                                  ]));
                            }).toList()),
                          )
                        ]
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          ...
                          children: [
                            Expanded(
                              child: ButtonFill(
                                  controller: GetXCreator.putButtonFillController('btnAgreeOrderRequest'),
                                  label: 'Yakin',
                                  onClick: () {
                                    ...
                                    AuthImpl().get().then((auth) {
                                      if (auth != null) {
                                        final String requestedDate = '${Convert.getYear(orderDateField.getLastTimeSelected())}-${Convert.getMonthNumber(orderDateField.getLastTimeSelected())}-${Convert.getDay(orderDateField.getLastTimeSelected())}';
                                        final Procurement requestBody = Procurement(type: isFeed ? 'pakan' : 'ovk', coopId: coop.id, farmingCycleId: coop.farmingCycleId, requestSchedule: requestedDate);
                                        ...
                                        if (isOwnFarm) {
                                          if (ovkUnitMultipleFormField.getController().listObjectAdded.isNotEmpty) {
                                            final Procurement internalRequest = Procurement(
                                                coopSourceId: '',
                                          ...
                                          requestBody.details = isFeed
                                              ? feedMultipleFormField.getController().listObjectAdded.entries.map((entry) => entry.value).toList().cast<Product>()
                                        ...
                                        if (isEdit) {
                                          _pushPurchaseRequestToServer(ListApi.updateOrderOrTransferRequest,
                                              ['Bearer ${auth.token}', auth.id, '${fromCoopRest ? "v2/purchase-requests/sapronak-doc-in/" : "v2/purchase-requests/"}${procurement.id}', Mapper.asJsonString(requestBody)]);
                                        } else {
                            ...
                            const SizedBox(width: 16),
                            Expanded(child: ButtonOutline(controller: GetXCreator.putButtonOutlineController('btnNotAgreeOrderRequest'), label: 'Tidak Yakin', onClick: () => Navigator.pop(Get.context!)))
                          ],
                        ),
                      ),
  ...
  Product getOvkUnitSelectedObject() {
    if (ovkUnitSuggestField.getController().getSelectedObject() != null) {
      final Product product = ovkUnitSuggestField.getController().getSelectedObject();
