...
import 'daily_report_detail_controller.dart';
...
class DailyReportDetailActivity extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final DailyReportDetailController controller = Get.put(DailyReportDetailController(context: context));

    Widget rowDetail(String title, String? value) {
      return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: GlobalVar.greyTextStyle.copyWith(fontSize: 12)), Text('$value', style: GlobalVar.blackTextStyle.copyWith(fontSize: 12, fontWeight: FontWeight.w500))]);
    }

    Widget consumptionDetail(String merek, String value) {
      return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Merek', style: GlobalVar.subTextStyle.copyWith(fontSize: 12)),
            const SizedBox(height: 4),
            Text(merek, style: GlobalVar.blackTextStyle.copyWith(fontSize: 14, fontWeight: FontWeight.w500)),
            const SizedBox(height: 16),
            Text('Total', style: GlobalVar.subTextStyle.copyWith(fontSize: 12)),
            const SizedBox(height: 4),
            Text(value, style: GlobalVar.blackTextStyle.copyWith(fontSize: 14, fontWeight: FontWeight.w500))
          ]));
    }
                      ...
                      child: SingleChildScrollView(
                          child: Container(
                              ...
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                if (controller.reportDetail != null && controller.reportDetail!.feedConsumptions!.isNotEmpty) ...[
                                  Column(children: controller.reportDetail!.feedConsumptions!.map((e) => consumptionDetail('${e!.subcategoryName} - ${e.productName!}', "${e.quantity} ${e.purchaseUom ?? e.uom ?? '-'}")).toList())
                                ] else ...[
                                  Text('-', style: GlobalVar.blackTextStyle.copyWith(fontWeight: FontWeight.w500, fontSize: 12))
                                ],
                                const SizedBox(height: 24),
                                Text('Konsumsi OVK', style: GlobalVar.blackTextStyle.copyWith(fontWeight: FontWeight.bold)),
                                if (controller.reportDetail != null && controller.reportDetail!.ovkConsumptions!.isNotEmpty) ...[
                                  Column(children: controller.reportDetail!.feedConsumptions!.map((e) => consumptionDetail('${e!.subcategoryName} - ${e.productName!}', "${e.quantity} ${e.purchaseUom ?? e.uom ?? '-'}")).toList())
                                ] else ...[
                                  Container(margin: const EdgeInsets.only(top: 16), child: Text('-', style: GlobalVar.blackTextStyle.copyWith(fontWeight: FontWeight.w500, fontSize: 12)))
                                ],
                                const SizedBox(height: 24),
                                  ...
                                  Column(
                                      children: controller.reportDetail!.images!
                                          .map((e) => Container(
                                              ...
                                              child: Image.network(e!.url!, fit: BoxFit.cover, loadingBuilder: (BuildContext context, Widget child, ImageChunkEvent? loadingProgress) {
