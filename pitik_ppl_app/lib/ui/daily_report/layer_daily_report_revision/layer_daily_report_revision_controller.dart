...
class LayerDailyReportRevisionController extends GetxController {
  ...
  EditField efOtherReasonRevision =
      EditField(controller: GetXCreator.putEditFieldController('layerDailyRevisionOtherReasonField'), label: '', hideLabel: true, hint: 'Ketik di sini', alertText: 'Harus diisi..!', textUnit: '', maxInput: 200, onTyping: (text, field) {});
  ...
  void onInit() {
    ...
    bfRevision = ButtonFill(
        controller: GetXCreator.putButtonFillController('layerDailyBtnRevision'),
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            SizedBox(
                                ...
                                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                  Expanded(
                                      child: ButtonFill(
                                          controller: GetXCreator.putButtonFillController('btnLayerDailyReportRevisionYes'),
                                          label: 'Yakin',
                                          onClick: () => AuthImpl().get().then((auth) {
                                                if (auth != null) {
                                                  Navigator.pop(Get.context!);
                                                  isLoading.value = true;

                                                  final DailyReportRevision bodyRequest = DailyReportRevision(reason: _getReasonRevision(), changes: revisionReasonList);
                                  ...
                                  Expanded(child: ButtonOutline(controller: GetXCreator.putButtonOutlineController('btnLayerDailyReportRevisionNo'), label: 'Tidak Yakin', onClick: () => Navigator.pop(Get.context!)))
                                ])),
                            const SizedBox(height: 32)
                          ])))));
        } else {
