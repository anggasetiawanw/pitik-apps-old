...
class DailyReportHomeActivity extends GetView<DailyReportHomeController> {
    ...
    Widget build(BuildContext context) {
            ...
            return GestureDetector(
                onTap: () {
                    if (report.status == EnumDailyReport.FILL_SOON) {
                        Get.toNamed(
                            controller.coop!.farmCategory == 'LAYER' ? RoutePage.layerDailyReportForm : RoutePage.dailyReportForm,
                            arguments: [controller.coop!, report])!.then((value) => controller.getDailyReport()
                        );
                    } else {
                        Get.toNamed(
                            controller.coop!.farmCategory == 'LAYER' ? RoutePage.layerDailyReportDetail : RoutePage.dailyReportDetail,
                            arguments: [controller.coop!, report])!.then((value) => controller.getDailyReport()
                        );
                    }
                },
                            ...
                            Expanded(
                                child: Column(
                                    ...
                                    children: [
                                            ...
                                            children: [
                                                Column(
                                                    ...
                                                    children: [
                                                        ...
                                                        Text("${report?.date}", style: GlobalVar.blackTextStyle.copyWith(fontSize: 12))
                                                    ]
                                                ),
                                        ...
                                        Text("Laporan harian ${report.status}", style: GlobalVar.greyTextStyle.copyWith(fontSize: 12))
                                    ]
                                )
                            )
                        ]
        ...
        return Scaffold(
            ...
            body: Column(
                children: [
                  Obx(() => controller.isLoading.isTrue ? const Center(child: ProgressLoading()) : Expanded(
                        child: RawScrollbar(
                            ...
                            child: RefreshIndicator(
                                ...
                                child: controller.isLoadingList.isTrue ? Padding(
                                    ...
                                    child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) => listCard(controller.reportList[index])
                                    )
                                )
                            )
                        )
