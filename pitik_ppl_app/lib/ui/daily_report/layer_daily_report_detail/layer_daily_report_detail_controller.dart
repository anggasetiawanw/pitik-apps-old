...
class LayerDailyReportDetailController extends GetxController {
  ...
  void onInit() {
    ...
    bfEdit = ButtonFill(
        controller: GetXCreator.putButtonFillController('layerDailyDetailEdit'),
        label: 'Edit',
        onClick: () {
  ...
  void generateProductCards({required List<Product?> productList, bool isFeed = false}) {
    final Column dataWidget = Column(
      ...
      if (productList[index] != null) {
        ...
        return Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(isFeed ? '${product.subcategoryName ?? ''} - ${product.productName ?? ''}' : product.productName ?? '', style: GlobalVar.subTextStyle.copyWith(fontSize: 14, fontWeight: GlobalVar.medium, color: GlobalVar.black)),
              const SizedBox(height: 12),
              if (isFeed) ...[
                Text('Total', style: GlobalVar.subTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.medium, color: GlobalVar.grayText)),
                const SizedBox(height: 4),
                Text('${product.quantity == null ? '' : product.quantity!.toStringAsFixed(0)} ${product.uom ?? product.purchaseUom ?? ''}', style: GlobalVar.subTextStyle.copyWith(fontSize: 14, fontWeight: GlobalVar.medium, color: GlobalVar.black))
              ] else ...[
