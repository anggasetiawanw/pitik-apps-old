...
import '../../route.dart';
import 'list_transfer_controller.dart';
import 'transfer_common.dart';
                  ...
                  : Expanded(
                      ...
                      RawScrollbar(
                          ...
                          child: RefreshIndicator(
                              onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => TransferCommon.getListSend(coop: controller.coop, isLoading: controller.isLoading, destinationTransferList: controller.transferList)),
                              child: ListView.builder(
                                  physics: const AlwaysScrollableScrollPhysics(),
                                  itemCount: controller.transferList.length,
                                  itemBuilder: (context, index) => TransferCommon.createTransferCard(coop: controller.coop, procurement: controller.transferList[index], onRefreshData: () => controller.refreshTransferList())))),
                      RawScrollbar(
                          thumbColor: GlobalVar.primaryOrange,
                          radius: const Radius.circular(8),
                          child: RefreshIndicator(
                            onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => TransferCommon.getListReceived(coop: controller.coop, isLoading: controller.isLoading, destinationTransferList: controller.transferList)),
                            child: ListView.builder(
                                physics: const AlwaysScrollableScrollPhysics(),
                                itemCount: controller.transferList.length,
                                itemBuilder: (context, index) => TransferCommon.createTransferCard(isGrTransfer: true, coop: controller.coop, procurement: controller.transferList[index], onRefreshData: () => controller.refreshTransferList())),
                          ))
                    ]))
            ],
          ),
