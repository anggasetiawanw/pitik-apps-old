...
class TransferDetailController extends GetxController {
  ...
  void onInit() {
    ...
    rejectReasonAreaField = EditAreaField(controller: GetXCreator.putEditAreaFieldController('transferRejectReason'), label: 'Alasan', hint: 'Tulis Alasan disini...', alertText: 'Alasan belum diisi..!', maxInput: 250, onTyping: (text, field) {});
    _getDetailRequest();
  }
  ...
  Container getStatus() {
    return Container(
      ...
      decoration: BoxDecoration(
          color: procurement.value.statusText == null
              ? Colors.transparent
              : procurement.value.statusText == GlobalVar.PENGAJUAN || procurement.value.statusText == GlobalVar.NEED_APPROVAL
                  ? GlobalVar.primaryLight2
                  : procurement.value.statusText == GlobalVar.REJECTED || procurement.value.statusText == GlobalVar.ABORT
                      ? GlobalVar.redBackground
                      : procurement.value.statusText == GlobalVar.DITERIMA || procurement.value.statusText == GlobalVar.APPROVED
                          ? GlobalVar.greenBackground
                          : procurement.value.statusText == GlobalVar.DIKIRIM
                              ? GlobalVar.blueBackground
                              : Colors.white,
          borderRadius: const BorderRadius.all(Radius.circular(7))),
      child: Text(procurement.value.statusText == null ? '-' : procurement.value.statusText!,
  ...
  Text getTransferDate() {
    if (procurement.value.deliveryDate != null) {
      final DateTime dateTime = Convert.getDatetime(procurement.value.deliveryDate!);
  ...
  Text getCoopTarget() {
    if (isSend) {
      return Text(procurement.value.coopTargetName == null || procurement.value.coopTargetName == '' ? procurement.value.branchTargetName! : procurement.value.coopTargetName!,
          style: GlobalVar.subTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.medium, color: GlobalVar.black));
    } else {
      return Text(procurement.value.coopSourceName ?? '', style: GlobalVar.subTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.medium, color: GlobalVar.black));
    }
  }

  Text getPurposeTransfer() {
    return Text(
        procurement.value.route == null
            ? ''
            : procurement.value.route == 'COOP-TO-BRANCH'
                ? 'Unit'
                : 'Kandang',
        style: GlobalVar.subTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.medium, color: GlobalVar.black));
  }
      ...
      if (auth != null) {
        Service.push(
            ...
            listener: ResponseListener(
                onResponseDone: (code, message, body, id, packet) {
                  isLoading.value = false;
                  Get.off(TransactionSuccessActivity(keyPage: 'transferApprove${procurement.value.id}', message: 'Kamu telah menyetujui permintaan transfer pakan', showButtonHome: false, onTapClose: () => Get.back(result: true), onTapHome: () {}));
                },
                onResponseFail: (code, message, body, id, packet) {
                  Get.snackbar('Pesan', '${(body as ErrorResponse).error!.message}', snackPosition: SnackPosition.TOP, colorText: Colors.white, backgroundColor: Colors.red);
                  isLoading.value = false;
  ...
  void _rejectTransfer({bool isCancel = false}) {
    if (rejectReasonAreaField.getInput().isNotEmpty) {
      ...
      AuthImpl().get().then((auth) {
        if (auth != null) {
          final String endpoint = isCancel ? 'v2/transfer-requests/${procurement.value.id}/cancel' : 'v2/transfer-requests/${procurement.value.id}/reject';
          Service.push(
              apiKey: 'productReportApi',
              ...
              body: [
                ...
                auth.id,
                endpoint,
                json.encode({'remarks': rejectReasonAreaField.getInput()})
              ],
              listener: ResponseListener(
                  onResponseDone: (code, message, body, id, packet) {
                    isLoading.value = false;
                      ...
                      child: Container(
                          ...
                          child: SizedBox(
                              ...
                              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                ...
                                Expanded(
                                    child: ButtonOutline(
                                        controller: GetXCreator.putButtonOutlineController('btnTransferReject'),
                                        label: 'Tolak',
                                        onClick: () {
                    ...
                    containerButtonEditAndCancel.value = SizedBox(
                      child: Container(
                          ...
                          child: SizedBox(
                              ...
                              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                Expanded(
                                  child: ButtonFill(controller: GetXCreator.putButtonFillController('btnTransferEdit'), label: 'Edit', onClick: () => Get.toNamed(RoutePage.transferRequestPage, arguments: [coop, true, procurement.value])),
                                ),
                                const SizedBox(width: 16),
                                Expanded(
                                    child: ButtonOutline(
                                        controller: GetXCreator.putButtonOutlineController('btnTransferCancel'),
                                        label: 'Batal',
                                        onClick: () {
