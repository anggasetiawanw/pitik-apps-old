...
import '../transfer/transfer_common.dart';
import 'farm_closing_controller.dart';
        ...
        child: Scaffold(
            ...
            body: Column(children: [
              Padding(
                ...
                child: Row(children: [
                  Stack(alignment: Alignment.centerLeft, children: [controller.barList[0], controller.pointList[0]]),
                  Stack(alignment: Alignment.centerLeft, children: [controller.barList[1], controller.pointList[1]]),
                  Stack(alignment: Alignment.centerLeft, children: [controller.isOwnFarm() ? controller.barList[2] : const SizedBox(), controller.pointList[2]]),
                      ...
                      ? Column(children: [
                          Padding(
                              ...
                              child: TabBar(
                                  ...
                                  tabs: const [Tab(text: 'Deal'), Tab(text: 'Realisasi')])),
                          const SizedBox(height: 8),
                          controller.isLoading.isTrue
                              ? const Padding(padding: EdgeInsets.only(top: 100), child: Center(child: ProgressLoading()))
                              : Expanded(
                                  child: TabBarView(controller: controller.harvestTabController, children: [
                                  RawScrollbar(
                                      thumbColor: GlobalVar.primaryOrange,
                                      radius: const Radius.circular(8),
                                      child: RefreshIndicator(
                                        onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => HarvestCommon.getDealList(isLoading: controller.isLoading, coop: controller.coop, harvestList: controller.harvestList)),
                                  ...
                                  RawScrollbar(
                                      ...
                                      radius: const Radius.circular(8),
                                      child: RefreshIndicator(
                                          onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => HarvestCommon.getRealizationList(isLoading: controller.isLoading, coop: controller.coop, realizationList: controller.realizationList)),
                                          child: ListView.builder(
                                              physics: const AlwaysScrollableScrollPhysics(),
                                              itemCount: controller.realizationList.length,
                                              itemBuilder: (context, index) => HarvestCommon.createRealizationHarvestCard(coop: controller.coop, realization: controller.realizationList[index], onRefreshData: () => controller.refreshHarvestList()))))
                                ]))
                        ])
                          ...
                          : controller.state.value == 2
                              ? controller.isLoading.isTrue
                                  ...
                                  : RawScrollbar(
                                      ...
                                      radius: const Radius.circular(8),
                                      child: RefreshIndicator(
                                          onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.toCheckFeed()),
                                          child: ListView(children: [
                                            Padding(
                                              ...
                                              child: GestureDetector(
                                                  ...
                                                  child: Container(
                                                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                                      decoration:
                                                          const BoxDecoration(color: GlobalVar.grayBackground, border: Border.fromBorderSide(BorderSide(color: GlobalVar.outlineColor, width: 2)), borderRadius: BorderRadius.all(Radius.circular(10))),
                                            ...
                                            const SizedBox(height: 16),
                                            controller.showFeedInformationBanner.isTrue
                                                ? Padding(
                                                    padding: const EdgeInsets.symmetric(horizontal: 16),
                                                    child: Container(
                                                        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                                        ...
                                                        child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                                          ...
                                                          const SizedBox(width: 12),
                                                          Expanded(
                                                              child: Text('Transfer pakan belum diterima oleh kandang tujuan',
                                                                  style: GlobalVar.subTextStyle.copyWith(fontSize: 12, fontWeight: GlobalVar.medium, color: GlobalVar.blue), overflow: TextOverflow.clip)),
                                                        ])),
                                                  )
                                                : const SizedBox(),
                                            const SizedBox(height: 16),
                                            Column(
                                                children: List.generate(controller.feedTransferList.length,
                                                    (index) => TransferCommon.createTransferCard(coop: controller.coop, procurement: controller.feedTransferList[index], onRefreshData: () => controller.toCheckFeed())))
                                          ])))
                              : controller.state.value == 3
                                  ? controller.isLoading.isTrue
                                      ...
                                      : RawScrollbar(
                                          ...
                                          radius: const Radius.circular(8),
                                          child: RefreshIndicator(
                                              onRefresh: () => Future.delayed(const Duration(milliseconds: 200), () => controller.toCheckOvk()),
                                              child: ListView(children: [
                                                Padding(
                                                  padding: const EdgeInsets.symmetric(horizontal: 16),
                                                  child: GestureDetector(
                                                ...
                                                const Divider(height: 2, color: GlobalVar.gray),
                                                const SizedBox(height: 16),
                                                controller.showOvkInformationBanner.isTrue
                                                    ? Padding(
                                                        padding: const EdgeInsets.symmetric(horizontal: 16),
                                                        child: Container(
                                                            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                                ...
                                                const SizedBox(height: 16),
                                                Column(
                                                    children: List.generate(controller.ovkTransferList.length,
                                                        (index) => TransferCommon.createTransferCard(coop: controller.coop, procurement: controller.ovkTransferList[index], onRefreshData: () => controller.toCheckOvk())))
                                              ])))
                                  : const SizedBox())
            ]))));
  }
