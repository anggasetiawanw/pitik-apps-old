...
class EditDataPurchaseController extends GetxController {
    ...
    Rx<List<OperationUnitModel?>> listSourceJagal =
        Rx<List<OperationUnitModel>>([]);
    Rx<List<OperationUnitModel?>> listDestinationPurchase =
        Rx<List<OperationUnitModel>>([]);
    ...
    void onInit() {
        ...
        controller:
            InternalControllerCreator.putSkuCardPurchaseController("skuPurchase", context),
        );
    ...
    void generateListProduct(int idx) {
        ...
        // skuCard.controller.spinnerSku.value[idx].controller.generateItems(mapList.value);
    ...
    void getListSourceVendor() {
        ...
        Service.push(
            ...
            Constant.auth!.id,
            Constant.xAppId!,
            AppStrings.TRUE_LOWERCASE
            ],
            listener:
                ResponseListener(
                onResponseDone: (code, message, body, id, packet) {
                    Map<String, bool> mapList = {};
                    for (var vendor in (body as VendorListResponse)
                        .data) {
        ...
        if (listProduct.isNotEmpty) {
                ...
                Timer(Duration.zero, () async {
                    ...
                    sku.controller.spinnerCategories.value[j].controller
                        .setTextSelected(listProduct[j]!.category!.name!);
                    sku.controller.spinnerCategories.value[j].controller
                        .generateItems(listKebutuhan);
                    sku.controller.spinnerSku.value[j].controller
                        .setTextSelected(listProduct[j]!.name!);
                    sku.controller.editFieldJumlahAyam.value[j]
                        .setInput(listProduct[j]!.quantity!.toString());
                    sku.controller.editFieldKebutuhan.value[j]
                        .setInput(listProduct[j]!.weight!.toString());
                    sku.controller.editFieldHarga.value[j]
                        .setInput(listProduct[j]!.price!.toString());
                    sku.controller.listSku.value[j] = listProduct;
                    sku.controller.mapSumChick[j] =
                        sku.controller.editFieldJumlahAyam.value[j].getInputNumber()!;
                    sku.controller.mapSumNeeded[j] =
                        sku.controller.editFieldKebutuhan.value[j].getInputNumber()!;
                    sku.controller.mapSumPrice[j] =
                    ...
                    Timer(const Duration(milliseconds: 500), () {
                        ...
                        if (sku.controller.spinnerCategories.value[j].controller
                                    .textSelected.value ==
                                AppStrings.LIVE_BIRD ||
                            sku.controller.spinnerCategories.value[j].controller
                                    .textSelected.value ==
                                AppStrings.AYAM_UTUH ||
                            sku.controller.spinnerCategories.value[j].controller
                                    .textSelected.value ==
                                AppStrings.BRANGKAS) {
                        sku.controller.editFieldJumlahAyam.value[j].controller
                            .enable();
                        }
                        sku.controller.editFieldKebutuhan.value[j].controller.enable();
                        sku.controller.editFieldHarga.value[j].controller.enable();
    ...
    void editPurchase() {
        ...
        Service.push(
            ...
            Constant.auth!.id,
            Constant.xAppId!,
            ListApi.pathEditPurchase(purchaseDetail.id!),
    ...
    List validation() {
        ...
        Scrollable.ensureVisible(
            spinnerSource.controller.formKey.currentContext!);
        return ret = [false, ""];
        }
        if (spinnerDestination.controller.textSelected.value.isEmpty) {
        spinnerDestination.controller.showAlert();
        Scrollable.ensureVisible(
            spinnerDestination.controller.formKey.currentContext!);
