...
import 'data_visit_controller.dart';
...
class VisitActivity extends GetView<VisitController> {
  ...
  Widget build(BuildContext context) {
    final VisitController controller = Get.put(
    ...
    Widget detailBisnis() {
      return Container(
        decoration: BoxDecoration(color: const Color(0xFFFAFAFA), border: Border.all(width: 1, color: AppColors.grey), borderRadius: BorderRadius.circular(8)),
        ...
        child: Column(
          children: [
            ...
            tileDetail(
              'Nama Bisnis',
              controller.customer.value != null && controller.customer.value!.businessName != null ? '${controller.customer.value!.businessName}' : '-',
            ),
            tileDetail(
              'Tipe Bisnis',
              controller.customer.value != null && controller.customer.value!.businessType != null ? '${controller.customer.value!.businessType}' : '-',
            ),
            tileDetail(
              'Nama Pemilik',
              controller.customer.value != null && controller.customer.value!.ownerName != null ? '${controller.customer.value!.ownerName}' : '-',
            ),
            tileDetail('Nomor Telepon ', controller.customer.value != null && controller.customer.value!.phoneNumber != null ? '${controller.customer.value!.phoneNumber}' : '-'),
            tileDetail('Kota', controller.customer.value != null && controller.customer.value!.city != null ? '${controller.customer.value!.city!.name}' : '-'),
            tileDetail('Kecamatan', controller.customer.value != null && controller.customer.value!.district != null ? '${controller.customer.value!.district!.name}' : '-'),
          ],
        ),
      );
    }
                  ...
                  ? Container(
                      ...
                      child: ListView(
                        children: [
                          ...
                          controller.isLoadCheckin.isFalse ? controller.checkinButton : const Center(child: ProgressLoading()),
