...
import 'data_visit_activity.dart';
...
class VisitController extends GetxController {
  ...
  final EditField editAlasan = EditField(controller: GetXCreator.putEditFieldController('alasanLama'), label: 'Alasan*', hint: 'Ketik Disini', alertText: 'Kolom ini harus diisi!', textUnit: '', maxInput: 100, onTyping: (value, controller) {});
  ...
  late ButtonFill buttonFillSelesai = ButtonFill(
    ...
    onClick: () {
      ...
      if (ret[0]) {
        final VisitCustomer visPayload = generatePayload();
        ...
        if (ret[1] != null) {
          if ((ret[1] as String).isNotEmpty) {
            Get.snackbar('Pesan', 'Duplikat Item Produk, ${ret[1]}', snackPosition: SnackPosition.BOTTOM, backgroundColor: const Color(0xFFFF0000), colorText: const Color(0xFFFFFFFF));
          }
        }
      }
    },
  ...
  late SpinnerSearch spinnerProvince = SpinnerSearch(
    ...
    onSpinnerSelected: (text) {
      if (province.value.isNotEmpty) {
        provinceSelect = province.value.firstWhereOrNull((element) => element!.provinceName! == text);
          ...
          await FlLocation.getLocation(timeLimit: timeLimit, accuracy: LocationAccuracy.high).then((position) {
            if (position.isMock) {
              ...
              Service.push(
                  ...
                  context: context,
                  body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, ListApi.pathCheckinVisit(customer.value!.id!), Mapper.asJsonString(CheckInModel(latitude: position.latitude, longitude: position.longitude))],
                  listener: ResponseListener(
                      onResponseDone: (code, message, body, id, packet) {
    ...
    spinnerMulti.getController().selectedValue.listen((p0) {
      if (p0.contains('Harga')) {
        ...
        for (int i = 0; i < skuCard.controller.itemCount.value; i++) {
          skuCard.controller.spinnerProduct.value[i].controller
            ..showLoading()
            ..disable();
        }
        getProduct();
  ...
  VisitCustomer generatePayload() {
      ...
      for (int i = 0; i < skuCard.controller.itemCount.value; i++) {
        ...
        if (listCategories.value.isNotEmpty) {
          ...
          if (selectCategory == null) {
            final Products? selectTemp = customer.value!.products!.firstWhereOrNull((element) => element!.category!.name! == skuCard.controller.spinnerProduct.value[whichItem].controller.textSelected.value);
        ...
        if (skuCard.controller.listProduct.value.isNotEmpty) {
          for (int j = 0; j < listProductTemp.length; j++) {
            selectProduct = listProductTemp[j].firstWhereOrNull(
              (element) => element!.name! == skuCard.controller.spinnerSize.value[whichItem].controller.textSelected.value,
            );
          }
    ...
    final List<OrderIssueCategories?>? categories = [];

    for (var spinner in spinnerMulti.controller.selectedValue.value) {
      final OrderIssueCategories? select = orderIssueCategories.value.firstWhereOrNull((list) => list!.title! == spinner);
    ...
    return VisitCustomer(
      ...
      orderIssue: spinnerKendala.controller.textSelected.value.isNotEmpty
          ? spinnerKendala.controller.textSelected.value == 'Ya'
              ...
              : false
          : null,
      orderIssueCategories: spinnerKendala.controller.textSelected.value == 'Ya' ? categories : [],
      ...
      products: spinnerMulti.controller.selectedValue.value.contains('Harga') ? listProductPayload : [],
    );
  }
}
