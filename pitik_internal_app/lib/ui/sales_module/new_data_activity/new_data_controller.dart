...
class NewDataController extends GetxController {
    ...
    onSpinnerSelected: (text) {
      try {
        if (province.value.isNotEmpty) {
          final Location? selectLocation = province.value.firstWhere((element) => element!.provinceName! == text);
    ...
    onSpinnerSelected: (text) {
      try {
        if (city.value.isNotEmpty) {
          final Location? selectLocation = city.value.firstWhere((element) => element!.cityName! == text);
          if (selectLocation != null) {
  ...
  void countFetchAPI() {
    ...
    if (countFetch == 3) {
      ...
      skuCard.controller.spinnerProduct.value[0].controller.generateItems(mapList.value);
  ...
  void getProvince() {
    ...
    Service.pushWithIdAndPacket(
        service: ListApi.getProvince, context: context, id: 1, packet: [isLoadProvince, province, spinnerProvince, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!], listener: _getListLocationListener);
  }

  void getCity(Location province) {
    // isLoadCity.value = true;
    Service.pushWithIdAndPacket(
        service: ListApi.getCity, context: context, id: 2, packet: [isLoadCity, city, spinnerKota, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, province.id], listener: _getListLocationListener);
  }

  void getDistrict(Location city) {
    // isLoadDistrict.value = true;
    Service.pushWithIdAndPacket(
        service: ListApi.getDistrict, context: context, id: 3, packet: [isLoadDistrict, district, spinnerKecamatan, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, city.id], listener: _getListLocationListener);
  }
  ...
  void getProduct() {
    Service.push(
      ...
      listener: ResponseListener(
          ...
          onResponseFail: (code, message, body, id, packet) {
            Get.snackbar('Alert', body.error!.message!, snackPosition: SnackPosition.BOTTOM, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
          },
          onResponseError: (exception, stacktrace, id, packet) {
            Get.snackbar('Alert', 'Terjadi kesalahan internal', snackPosition: SnackPosition.BOTTOM, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
          },
          onTokenInvalid: Constant.invalidResponse()),
    );
  }
  ...
  void saveCustomer(bool isYesButton) {
    ...
    if (ret[0]) {
      isLoading.value = true;
      try {
        final Customer cusPayload = generatePayload(true);
        Service.push(
          apiKey: 'userApi',
        ...
        Get.snackbar('ERROR', 'Error : $e \n Stacktrace->$st', snackPosition: SnackPosition.BOTTOM, duration: const Duration(seconds: 5), backgroundColor: const Color(0xFFFF0000), colorText: const Color(0xFFFFFFFF));
      }
    } else {
      if (ret[1] != null) {
  ...
  Customer generatePayload(bool isyes) {
    ...
    for (int i = 0; i < skuCard.controller.itemCount.value; i++) {
      ...
      final CategoryModel? selectCategory = listCategories.value.firstWhere((element) => element!.name! == skuCard.controller.spinnerProduct.value[whichItem].controller.textSelected.value);
    ...
    final Branch? branchSelected = listBranch.value.firstWhere((element) => element!.name == spBranch.controller.textSelected.value);

    final Location? provinceSelect = province.value.firstWhere(
      (element) => element!.provinceName == spinnerProvince.controller.textSelected.value,
    );

    final Location? citySelect = city.value.firstWhere(
      (element) => element!.cityName == spinnerKota.controller.textSelected.value,
    );

    final Location? districSelect = district.value.firstWhere(
