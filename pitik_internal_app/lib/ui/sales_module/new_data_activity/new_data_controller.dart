...
class NewDataController extends GetxController {
    ...
    onSpinnerSelected: (text) {
      try {
        if (province.value.isNotEmpty) {
          Location? selectLocation = province.value.firstWhereOrNull((element) => element!.provinceName! == text);
    ...
    onSpinnerSelected: (text) {
      try {
        if (city.value.isNotEmpty) {
          Location? selectLocation = city.value.firstWhereOrNull((element) => element!.cityName! == text);
          if (selectLocation != null) {
  ...
  void getProvince() {
    ...
    Service.pushWithIdAndPacket(service: ListApi.getProvince, context: context, id: 1, packet: [isLoadProvince, province, spinnerProvince, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!], listener: _getListLocationListener);
  }

  void getCity(Location province) {
    // isLoadCity.value = true;
    Service.pushWithIdAndPacket(service: ListApi.getCity, context: context, id: 2, packet: [isLoadCity, city, spinnerKota, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, province.id], listener: _getListLocationListener);
  }

  void getDistrict(Location city) {
    // isLoadDistrict.value = true;
    Service.pushWithIdAndPacket(service: ListApi.getDistrict, context: context, id: 3, packet: [isLoadDistrict, district, spinnerKecamatan, isLoading], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, city.id], listener: _getListLocationListener);
  }
  ...
  Customer generatePayload(bool isyes) {
    ...
    for (int i = 0; i < skuCard.controller.itemCount.value; i++) {
      ...
      CategoryModel? selectCategory = listCategories.value.firstWhereOrNull((element) => element!.name! == skuCard.controller.spinnerProduct.value[whichItem].controller.textSelected.value);
      Products? selectProduct;
      selectProduct = listProductTemp[whichItem].firstWhereOrNull((element) => element!.name! == skuCard.controller.spinnerSize.value[whichItem].controller.textSelected.value);
    ...
    Location? provinceSelect = province.value.firstWhereOrNull(
      (element) => element!.provinceName == spinnerProvince.controller.textSelected.value,
    );

    Location? citySelect = city.value.firstWhereOrNull(
      (element) => element!.cityName == spinnerKota.controller.textSelected.value,
    );

    Location? districSelect = district.value.firstWhereOrNull(
