...
import '../../../widget/sku_card/sku_card.dart';
import '../../../widget/sku_card/sku_card_controller.dart';
...
class EditDataController extends GetxController {
  ...
  late SpinnerField spinnerTipeBisnis = SpinnerField(
    ...
    items: const {'Rumah Makan': false, 'Warung': false, 'Restoran': false, 'Pedagang Keliling': false, 'Catering': false, 'Non-Bisnis': false},
    onSpinnerSelected: (text) {},
  );
  ...
  late SpinnerSearch spinnerProvince = SpinnerSearch(
    ...
    onSpinnerSelected: (text) {
      if (province.value.isNotEmpty) {
        final Location? selectLocation = province.value.firstWhereOrNull((element) => element!.provinceName! == text);
  ...
  late SpinnerSearch spinnerKota = SpinnerSearch(
    ...
    onSpinnerSelected: (text) {
      if (city.value.isNotEmpty) {
        final Location? selectLocation = city.value.firstWhereOrNull((element) => element!.cityName! == text);
        if (selectLocation != null) {
  ...
  void onInit() {
    ...
    simpanButton = ButtonFill(controller: GetXCreator.putButtonFillController('simpanButton'), label: 'Simpan', onClick: () => saveCustomer());
  }
  ...
  void onReady() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Get.find<SkuCardController>(tag: 'editCardController').itemCount.listen((p0) => generateListProduct(p0));
      getProvince();
      getBranch();
      getProduct();
      getSalesList();
  ...
  void getProvince() {
    ...
    Service.pushWithIdAndPacket(service: ListApi.getProvince, context: context, id: 1, packet: [isLoadApi, province, spinnerProvince], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!], listener: _getListLocationListener);
  }

  void getCity(Location province) {
    isLoadApi.value = true;
    Service.pushWithIdAndPacket(service: ListApi.getCity, context: context, id: 2, packet: [isLoadApi, city, spinnerKota], body: [Constant.auth!.token, Constant.auth!.id, province.id, Constant.xAppId!], listener: _getListLocationListener);
  }

  void getDistrict(Location city) {
    isLoadApi.value = true;
    Service.pushWithIdAndPacket(service: ListApi.getDistrict, context: context, id: 3, packet: [isLoadApi, district, spinnerKecamatan], body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, city.id], listener: _getListLocationListener);
  }
    ...
    Service.push(
      ...
      listener: ResponseListener(
          onResponseDone: (code, message, body, id, packet) {
            ...
            skuCard.controller.spinnerProduct.value[0].controller.generateItems(mapList);
  ...
  void saveCustomer() {
    ...
    if (ret[0]) {
      isLoading.value = true;
      final Customer cusPayload = generatePayload();
      Service.push(
        apiKey: 'userApi',
      ...
      if (ret[1] != null) {
        if ((ret[1] as String).isNotEmpty) {
          Get.snackbar('Pesan', 'Duplikat Item Produk, ${ret[1]}', snackPosition: SnackPosition.BOTTOM, duration: const Duration(seconds: 5), backgroundColor: const Color(0xFFFF0000), colorText: const Color(0xFFFFFFFF));
        }
      }
    }
  }
  ...
  List<dynamic> validation() {
    ...
    if (editNamaBisnis.getInput().isEmpty) {
      ...
      Get.snackbar('Alert', 'Nama Bisnis harus di isi!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (spinnerTipeBisnis.controller.textSelected.value.isEmpty) {
      ...
      Get.snackbar('Alert', 'Tipe Bisnis harus di pilih!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (editNamaPemilik.getInput().isEmpty) {
      ...
      Get.snackbar('Alert', 'Nama Pemilik harus di isi!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (editNomorTelepon.getInput().isEmpty) {
      ...
      Get.snackbar('Alert', 'Nomor Telepon harus di isi!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
      ...
      if (RegExp(
            r'^0([2-9]{1})([0-9]{7,11})',
            caseSensitive: false,
          ).hasMatch(mobilePhone) ==
          false) {
        editNomorTelepon.controller.setAlertText('Nomor telepon tidak valid!');
        editNomorTelepon.controller.showAlert();
        editNomorTelepon.controller.focusNode.requestFocus();
        Get.snackbar('Alert', 'Nomor Telepon Tidak Valid!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (editLokasiGoogle.getInput().isEmpty) {
      ...
      Get.snackbar('Alert', 'Lokasi Google harus di isi!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
      ...
      final bool rets = gpc.isValid(getCode[0]);
      if (!rets) {
        editLokasiGoogle.controller.setAlertText('Lokasi Google Tidak Valid');
        editLokasiGoogle.controller.showAlert();
        editLokasiGoogle.controller.focusNode.requestFocus();
        Get.snackbar('Alert', 'Lokasi Google tidak valid!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (spinnerProvince.controller.textSelected.value.isEmpty) {
      ...
      Get.snackbar('Alert', 'Province harus di pilih!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);

      return ret = [false, ''];
    }
    if (spinnerKota.controller.textSelected.value.isEmpty) {
      spinnerKota.controller.showAlert();
      Get.snackbar('Alert', 'Kota harus di pilih!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);

      return ret = [false, ''];
    }
    if (spinnerKecamatan.controller.textSelected.value.isEmpty) {
      spinnerKecamatan.controller.showAlert();
      Get.snackbar('Alert', 'Kecamatan harus di pilih!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
    ...
    if (spinnerSupplier.controller.textSelected.value.isEmpty) {
      ...
      Get.snackbar('Alert', 'Supplier harus di pilih!', snackPosition: SnackPosition.TOP, duration: const Duration(seconds: 5), backgroundColor: Colors.red, colorText: Colors.white);
  ...
  Customer generatePayload() {
    ...
    final listProductTemp = skuCard.controller.listProduct.value.values.toList();
    for (int i = 0; i < skuCard.controller.itemCount.value; i++) {
      ...
      if (listCategories.value.isNotEmpty) {
        ...
        if (selectCategory == null) {
          final Products? selectTemp = customer.products!.firstWhereOrNull((element) => element!.category!.name! == skuCard.controller.spinnerProduct.value[whichItem].controller.textSelected.value);
      ...
      if (skuCard.controller.listProduct.value.isNotEmpty) {
        ...
        selectProduct ??= customer.products!.firstWhereOrNull((element) => element!.name == skuCard.controller.spinnerSize.value[whichItem].controller.textSelected.value);
      } else {
        selectProduct = customer.products!.firstWhereOrNull((element) => element!.name == skuCard.controller.spinnerSize.value[whichItem].controller.textSelected.value);
      }
    ...
    if (customer.district!.name != spinnerKecamatan.controller.textSelected.value) {
      districSelect = district.value.firstWhereOrNull(
    ...
    if (isSalesLead.isTrue) {
      salesSelect = listSalesperson.value.firstWhereOrNull(
        (element) => element!.email == spinnerPicSales.controller.textSelected.value,
      );
    }
