...
class TransferFormController extends GetxController {
  ...
  late SpinnerField sourceField = SpinnerField(
      controller: GetXCreator.putSpinnerFieldController('sourceTransfer'),
      label: 'Sumber*',
  ...
  late SpinnerField destinationField = SpinnerField(
      controller: GetXCreator.putSpinnerFieldController('destinationTransfer'),
      label: 'Tujuan*',
      hint: 'Pilih salah satu',
  ...
  late SpinnerField categorySKUField = SpinnerField(
      controller: GetXCreator.putSpinnerFieldController('categorySKU'),
      label: 'Kategori SKU*',
      hint: 'Pilih salah satu',
      ...
      onSpinnerSelected: (value) {
        if (sourceSelect != null) {
          ...
          final Products? products = sourceSelect!.purchasableProducts!.firstWhereOrNull((element) => element!.name! == value);
          if (products != null) {
            final Map<String, bool> mapList = {};
            for (var element in products.productItems!) {
              mapList[element!.name!] = false;
            }
  ...
  EditField amountField = EditField(
    controller: GetXCreator.putEditFieldController('amountBirds'),
    label: 'Jumlah Ekor*',
    hint: '0',
  ...
  EditField totalField = EditField(
    controller: GetXCreator.putEditFieldController('totalFields'),
    label: 'Total*',
    hint: '0.0',
  ...
  void onReady() {
    ...
    if (isEdit.value) {
      ...
      if (categorySKUField.controller.textSelected.value == AppStrings.AYAM_UTUH ||
          categorySKUField.controller.textSelected.value == AppStrings.BRANGKAS ||
          categorySKUField.controller.textSelected.value == AppStrings.LIVE_BIRD ||
          categorySKUField.controller.textSelected.value == AppStrings.KARKAS) {
        amountField.controller.enable();
            ...
            onResponseDone: (code, message, body, id, packet) {
              ...
              if (isEdit.isTrue) {
                ...
                if (sourceSelect != null) {
                  ...
                  final Products? products = sourceSelect!.purchasableProducts!.firstWhereOrNull((element) => element!.name! == transferModel!.products![0]!.name);
                  Timer(const Duration(milliseconds: 500), () {
                    final Map<String, bool> mapList2 = {};
                    for (var element in products!.productItems!) {
                      mapList2[element!.name!] = false;
                    }
  ...
  TransferModel generatePayload(String status) {
    final OperationUnitModel? selectSource = listSourceOperationUnits.value.firstWhereOrNull((element) => element!.operationUnitName == sourceField.controller.textSelected.value);
    final OperationUnitModel? selectDestination = listDestinationOperationUnits.value.firstWhereOrNull((element) => element!.operationUnitName == destinationField.controller.textSelected.value);
    final Products? selectProduct = sourceSelect!.purchasableProducts!.firstWhereOrNull(
      (element) => element!.name == categorySKUField.controller.textSelected.value,
    );
    final Products? selectItem = selectProduct!.productItems!.firstWhereOrNull((element) => element!.name == skuField.controller.textSelected.value);
    return TransferModel(sourceOperationUnitId: selectSource!.id, targetOperationUnitId: selectDestination!.id, status: status, remarks: Uri.encodeFull(efRemark.getInput()), products: [
      Products(
