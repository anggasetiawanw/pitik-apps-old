...
class CreateGrTransferController extends GetxController {
  ...
  late ButtonFill yesSendButton = ButtonFill(
      controller: GetXCreator.putButtonFillController('yesSendGrTransferButton'),
      label: 'Ya',
      onClick: () {
        Get.back();
        saveGrTransfer();
  ...
  ButtonOutline noSendButton = ButtonOutline(
      controller: GetXCreator.putButtonOutlineController('noSendGrTransferButton'),
      label: 'Tidak',
      onClick: () {
        Get.back();
      });
  ...
  void saveGrTransfer() {
    isLoading.value = true;
    final PurchaseRequest transferPayload = generatePayload();
    Service.push(
      service: ListApi.createGoodReceived,
      context: context,
      body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId, Mapper.asJsonString(transferPayload)],
      listener: ResponseListener(
          onResponseDone: (code, message, body, id, packet) {
            isLoading.value = false;
            Get.back();
