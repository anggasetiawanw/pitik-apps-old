...
class DeliveryConfirmSOController extends GetxController {
  ...
  var error = ''.obs;
  late ButtonFill confirButton = ButtonFill(
      controller: GetXCreator.putButtonFillController('confirDeliveryConfirm'),
      label: 'Konfirmasi',
      onClick: () {
        _showBottomDialog();
      });

  SpinnerField paymentMethod = SpinnerField(
      controller: GetXCreator.putSpinnerFieldController('paymentDelivery'),
      label: 'Metode Pembayaran*',
      hint: 'Pilih Salah Satu',
      alertText: 'Harus dipilih salah satu!',
      items: const {'Tunai': false, 'Transfer': false},
      onSpinnerSelected: (value) {});
  EditField nominalMoney = EditField(
      controller: GetXCreator.putEditFieldController('nominalDelivery'),
      label: 'Nominal Uang*',
      hint: 'Tulis Jumlah',
      alertText: 'Nominal Uang harus diisi!',
      textUnit: '',
      maxInput: 20,
          ...
          await FlLocation.getLocation(timeLimit: timeLimit, accuracy: LocationAccuracy.high).then((position) {
            if (position.isMock) {
              ...
              Service.push(
                  ...
                  context: context,
                  body: [Constant.auth!.token, Constant.auth!.id, Constant.xAppId!, ListApi.pathCheckinDeliverySalesOrder(order.id!), Mapper.asJsonString(CheckInModel(latitude: position.latitude, longitude: position.longitude))],
  ...
  void onReady() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final Duration difference = timeEnd.difference(timeStart);
    ...
    if (validation()) {
      ...
      Service.push(
          ...
          body: [
            ...
            Constant.auth!.id,
            Constant.xAppId,
            ListApi.pathDeliveryConfirmSO(order.id!),
            Mapper.asJsonString(Order(
