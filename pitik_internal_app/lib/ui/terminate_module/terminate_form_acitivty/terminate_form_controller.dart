...
import '../../../widget/common/media_field.dart';
...
class TerminateFormController extends GetxController {
  ...
  late MediaField mediaField = MediaField(
    controller: GetXCreator.putMediaFieldController('photoPemusnahan'),
    onMediaResult: (value) {
      if (value != null) {
    ...
    label: '',
    hint: 'Upload Bukti Photo',
    alertText: 'Bukti Photo harus disertakan',
    showGallerOptions: false,
    type: 2,
  );
    ...
    Service.push(
        ...
        listener: ResponseListener(
            ...
            onResponseFail: (code, message, body, id, packet) {
              ...
              isLoading.value = false;
              categorySKUField.controller.hideLoading();
            },
            onResponseError: (exception, stacktrace, id, packet) {
              Get.snackbar(
                'Pesan',
  ...
  TerminateModel generatePayload(String status) {
    final OperationUnitModel? selectSource = listOperationUnits.value.firstWhereOrNull((element) => element!.operationUnitName == sourceField.controller.textSelected.value);
    ...
    return TerminateModel(
      ...
      imageLink: isEdit.isFalse
          ? mediaUploadData.value.url!
          : mediaField.controller.fileName.isNotEmpty
              ? mediaUploadData.value.url!
              : terminateModel!.imageLink,
    );
  }
