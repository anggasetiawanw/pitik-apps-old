...
import 'package:components/stock_opname_two_field/stock_opname_two_field.dart';
...
class StockOpnameController extends GetxController {
    ...
    Rx<List<StockOpnameTwoField>> listStockTwoField = Rx<List<StockOpnameTwoField>>([]);

    late SpinnerField sourceStock = SpinnerField(controller: GetXCreator.putSpinnerFieldController("stockSourceOpname"), label: "Sumber*", hint: "Pilih salah satu", alertText: "Sumber harus dipilih!", items: const {}, onSpinnerSelected: (value){
    ...
    showWidget(List<Products?> items, bool isEdit){
        ...
        listStockTwoField.value.clear();
        int idx = 0;
        int idx2 = 0;
        if(isEdit){
            for (var element in items) { 
                if(element!.name! == AppStrings.LIVE_BIRD || element.name! == AppStrings.AYAM_UTUH || element.name! == AppStrings.BRANGKAS){
                    listStockTwoField.value.add(StockOpnameTwoField(title: element.name!, controller: GetXCreator.putStockOpnameTwoField(element.name!)));
                    listStockTwoField.value[idx2].controller.generateEf(element);
                    listStockTwoField.refresh();
                    listStockTwoField.value[idx2].controller.setDefault(element);
                    idx2++;
                } else {
    ...
    OpnameModel generatePayload(String status){
        ...
        OperationUnitModel? selectSource = listOperationUnits.value.firstWhere((element) => element!.operationUnitName == sourceStock.controller.textSelected.value,);
        for(var stock in listStockField.value){
