...
class MediaField extends StatelessWidget {
  ...
  MediaField({required this.controller, required this.onMediaResult, required this.label, required this.hint, required this.alertText, super.key, this.hideLabel = false, this.type = ALL, this.showGallerOptions = true});
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color: controller.activeField.isTrue && controller.showTooltop.isFalse
                                      ? AppColors.primaryOrange
                                      : controller.activeField.isTrue && controller.showTooltop.isTrue
                                          ? AppColors.red
                                          : Colors.white,
                                  width: 2)),
                          child: Row(
                            children: [
                              Padding(padding: const EdgeInsets.only(right: 16, left: 8), child: SvgPicture.asset('images/icon_camera.svg')),
                              Expanded(
                                  child: Text(controller.fileName.value == '' ? hint : controller.fileName.value,
                                      overflow: TextOverflow.ellipsis, style: TextStyle(color: controller.activeField.isTrue ? AppColors.primaryOrange : AppColors.black, fontSize: 14)))
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _tileVideo(BuildContext context) {
    return ListTile(
      ...
      onTap: () async {
        ...
        final XFile? result = await ImagePicker().pickVideo(source: ImageSource.camera);

        if (result == null) {
          return onMediaResult(null);
        } else {
          final File fileResult = File(result.path);
  ...
  Widget _tileGalleryVideo(BuildContext context) {
    return ListTile(
      ...
      onTap: () async {
        Navigator.pop(context);
        final XFile? result = await ImagePicker().pickVideo(source: ImageSource.gallery);
